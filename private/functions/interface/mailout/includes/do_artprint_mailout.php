...
function generateMailoutEmailContent($replacements, $data, $m) {
    ...
    $text_body = $m->render("artprintTextTemplate", $replacements);
    $html_body = $m->render("artprintHtmlTemplate", $replacements);
...
// create log
$log_fp = fopen($log_dir . "artprint_list.log", 'a');
...
$mail->Subject = "add a signed artprint to your order";

try {
    $bodies = generateMailoutEmailContent(["heading"=>"add a signed artprint to your Almost Here vinyl order"], $row, $m);
